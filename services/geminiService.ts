 ...
 * Uses process.env.API_KEY as per strict environment requirements.
 */
export const matchSponsorsAI = async (params: {
  ...
  // Re-initializing per request to ensure latest API key usage as per guidelines
  const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
  
  try {
    // 1. Fetch live sponsor data from Supabase with robust error handling
    ...
    try {
      ...
      console.warn("Sponsors DB fetch failed during AI matching", dbErr);
      return [];
    }
    ...
    console.error("AI Matching Error (Gemini/Network):", error);
    return [];
  }
};
